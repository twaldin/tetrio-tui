...
async function main() {
  const term = await launchTerminal({
    command: 'npx', args: ['tsx', 'src/index.ts', '--guest', 'x', '--border-style', 'mixed'],
  ...
  fs.writeFileSync('/tmp/style_mixed2.png', await renderTerminalToImage((term as any).getTerminalData(), { fontSize: 14, devicePixelRatio: 2 }));
  console.log('captured mixed2');
